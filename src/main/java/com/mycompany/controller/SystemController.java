...
public class SystemController {
    ...
    public static class addItemForm {
    ...
    public ModelAndView newComment(Principal principal, ModelMap model, Form form,
        ...
        if (form.getAddComment() != "") {
            itemdesciption[productId][3] = itemdesciption[productId][3]
                    + principal.getName() + ": " + form.getAddComment() + "<br/>" ;
            model.addAttribute("productId", productId);
            model.addAttribute("products", products);
            model.addAttribute("itemdesciption", itemdesciption);
        }
    ...
    private void addToCart(HttpServletRequest request, HttpServletResponse response)
        ...
        if (itemdesciption[productId][2].equals("Yes")) {
    ...
    public ModelAndView edit(ModelMap model, HttpServletRequest request,
        ...
        productId = Integer.parseInt(request.getParameter("productId"));
        array = itemdesciption[productId][3].split("<br/>");
    ...
    public String updateItem(Principal principal, ModelMap model, ItemForm form,
       ...
       @GetMapping(value = {"", "/edit/{itemId}/deletecm/{cmid}"})
        public String deleteCm(@PathVariable("itemId") int itemId, @PathVariable("cmid") int cmId) 
                    throws AttachmentNotFound {
            String[] array;
            array = itemdesciption[itemId][3].split("<br/>");
            array[cmId] = "";
            String comments = ""; 
            
            for (String i : array) {
                if (i != ""){
                comments = comments + i + "<br/>";
                }   
            }
            System.out.print(comments);
            itemdesciption[itemId][3] = comments;
            
        return "redirect:/system/edit?productId="+itemId;
    }
        @GetMapping(value = {"", "/edit/{itemId}/deleteitem"})
        public String deleteitem(@PathVariable("itemId") int itemId) 
                    throws AttachmentNotFound {
        products.remove(itemId);
         attachmentService.deleteAttachmentbyItemid(new Long(itemId));

          //Delete the Model Map PART
        itemdesciption[itemId][0] = "";
        itemdesciption[itemId][1] = "";
        itemdesciption[itemId][2] = "";
        itemdesciption[itemId][3] = "";
        return "redirect:/system/index";
    }
    @GetMapping(value = {"", "/additem"})
    public ModelAndView create() {
        return new ModelAndView("additem", "product", new addItemForm());
    }
    
       @PostMapping("/additem")
        public View create(addItemForm form) throws IOException {
            String[] information = {form.getDescription(),form.getPrice(),form.getAvailability(),form.getComments()};
            int arraysize = itemdesciption.length;
            int hashtablesize = products.size()+1;
            itemdesciption = Arrays.copyOf(itemdesciption, itemdesciption.length + 1);
            itemdesciption[itemdesciption.length - 1] = information;
            products.put(hashtablesize,form.getItemname());
